...
<div class="score-picker" style="width:{{ $pickerWidth }}">
    ...
    <button type="button" class="score-picker-toggle form-control">
        <span class="sp-value">{{ $pickerValue !== '' ? $pickerValue : __('messages.select_placeholder') }}</span>
        <span class="sp-caret" aria-hidden="true">▾</span>
    </button>
    <div class="score-picker-list" hidden role="listbox">
        <div class="sp-option {{ $pickerValue === '' ? 'selected' : '' }}" data-value="" role="option">{{ __('messages.select_placeholder') }}</div>
        @for($v = 50; $v >= 10; $v--)
